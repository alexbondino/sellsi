...
function request(path, method='GET', body=null) {
  return new Promise((resolve, reject) => {
    ...
    if (body) {
      ...
      const req = https.request(opts, res => { let data = ''; res.on('data', c=> data+=c); res.on('end', ()=> resolve({ status: res.statusCode, body: data })); });
...
(async ()=>{
  ...
  try {
    ...
    const prod = JSON.parse(p.body)[0].productid;
    console.log('trying insert ONLY APIKEY... product:', prod);
