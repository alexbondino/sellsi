...
class UIController:
    ...
    def select_currency(self, currency="CLP"):
        ...
        try:
            self.log(
                f"[UI] Buscando dropdown de valorización para seleccionar '{currency}'...",
                level="INFO",
            )
            valorizar_box = None
            ...
            for intento in range(5):
                try:
                    valorizar_box = self.driver.find_element(
                        By.CSS_SELECTOR, "div.orange-font-color.dropdown-menu-label.mr-3"
                    )
                    break
                except Exception:
                    try:
                        valorizar_box = self.driver.find_element(
                            By.XPATH,
                            "//div[contains(@class, 'dropdown-menu-label') and (.//text()[contains(., 'Valorizar')] or .//text()[contains(., 'USD')] or .//text()[contains(., 'CLP')])]",
                        )
                        break
                    except Exception:
                        self.log(
                            f"[UI] Intento {intento+1}/5: Esperando dropdown...", level="DEBUG"
                        )
                        time.sleep(2)
                ...
                try:
                    ...
                    WebDriverWait(self.driver, 3).until(
                        lambda d: len(
                            d.find_elements(
                                By.XPATH,
                                "//*[contains(text(), 'CLP') and (contains(@class, 'dropdown-item') or contains(@class, 'option') or contains(@class, 'item'))]",
                            )
                        )
                        > 0
                    )
            ...
            try:
                ...
                WebDriverWait(self.driver, 5).until(
                    lambda d: any(
                        e.is_displayed() for e in d.find_elements(By.CSS_SELECTOR, ".dropdown-menu")
                    )
                )
            ...
            try:
                dropdowns = self.driver.find_elements(
                    By.XPATH, "//*[contains(@class, 'dropdown') or contains(@class, 'menu')]"
                )
                self.log(f"[DEBUG UI] Dropdowns/menus visibles: {len(dropdowns)}", level="DEBUG")
                for i, d in enumerate(dropdowns[:5]):
                    self.log(
                        f"[DEBUG UI] Dropdown {i+1}: tag={d.tag_name}, class={d.get_attribute('class')}, visible={d.is_displayed()}",
                        level="DEBUG",
                    )
                clp_candidates = self.driver.find_elements(By.XPATH, "//*[contains(text(), 'CLP')]")
                self.log(
                    f"[DEBUG UI] Elementos con texto 'CLP': {len(clp_candidates)}", level="DEBUG"
                )
                for i, c in enumerate(clp_candidates[:5]):
                    self.log(
                        f"[DEBUG UI] CLP {i+1}: tag={c.tag_name}, class={c.get_attribute('class')}, visible={c.is_displayed()}",
                        level="DEBUG",
                    )
            except Exception as e:
            ...
            for intento in range(5):
                ...
                if clp_encontrado:
                    break
                self.log(
                    f"[UI] Intento {intento+1}/5: No se encontró la opción '{currency}'. Reintentando...",
                    level="ADVERTENCIA",
                )
                time.sleep(2)
    ...
    def select_date(self, date_string):
        ...
        try:
            ...
            for intento in range(5):
                try:
                    year_btn = self.driver.find_element(
                        By.CSS_SELECTOR, "button.headerlabelbtn.yearlabel"
                    )
            ...
            for intento in range(5):
                try:
                    month_btn = self.driver.find_element(
                        By.CSS_SELECTOR, "button.headerlabelbtn.monthlabel"
                    )
            ...
            try:
                ...
                for i, d in enumerate(daycells[:10]):
                    span = (
                        d.find_element(By.TAG_NAME, "span")
                        if d.find_elements(By.TAG_NAME, "span")
                        else None
                    )
                    valor = span.text if span else "(sin span)"
                    self.log(
                        f"[DEBUG UI] daycell {i+1}: class={d.get_attribute('class')}, valor={valor}, enabled={d.is_enabled()}, visible={d.is_displayed()}",
                        level="DEBUG",
                    )
            except Exception as e:
