...
const calculateIsLate = (order) => {
  ...
  // Normalizar status display a backend key para comparación
  ...
  const backendStatus = normalizeStatus(order.status)
  const excludedStatuses = ['delivered', 'cancelled', 'rejected']
  return currentDate > endDate && !excludedStatuses.includes(backendStatus)
}
...
export const useOrdersStore = create((set, get) => ({
  ...
  fetchOrders: async (filters = {}) => {
    ...
    try {
      ...
      const visibilityFiltered = await orderService.getOrdersForSupplier(
        supplierId,
        filters
      )
      ...
      const processedOrders = visibilityFiltered.map((order) => {
        ...
        if (/no especificad/i.test(normalizedAddr.region))
          normalizedAddr.region = ''
        if (/no especificad/i.test(normalizedAddr.commune))
          normalizedAddr.commune = ''
      ...
      const sorted = [...processedOrders].sort((a, b) => {
        const da = new Date(
          a.created_at || a.requestedDate?.start || 0
        ).getTime()
        const db = new Date(
          b.created_at || b.requestedDate?.start || 0
        ).getTime()
  ...
  searchOrders: async (searchText) => {
    ...
    try {
      ...
      const visibilityFiltered = searchResults.filter(
        (o) =>
          o.payment_status === undefined ||
  ...
  subscribeRealtime: () => {
    ...
    if (typeof window !== 'undefined' && window.__ordersRealtimeSubscribed)
      return
    try {
