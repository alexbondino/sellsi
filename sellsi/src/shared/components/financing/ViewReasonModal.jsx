...
const ViewReasonModal = ({ open, financing, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (open && financing) {
      reasonRef.current = financing.paused_reason || financing.rejected_reason || financing.cancelled_reason || 'No hay motivo registrado';
    }
  }, [open, financing]);

  // Usar el valor actual de financing si existe, sino usar el ref preservado
  const reason = (financing?.paused_reason || financing?.rejected_reason || financing?.cancelled_reason)
    ? (financing.paused_reason || financing.rejected_reason || financing.cancelled_reason)
