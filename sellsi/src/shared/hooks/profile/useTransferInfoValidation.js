...
export const useTransferInfoValidation = (userId = null) => {
  ...
  const validateTransferInfo = useCallback((data) => {
    if (!data) {
      return {
        isComplete: false,
        missingFields: ['accountHolder', 'bank', 'accountNumber', 'transferRut', 'confirmationEmail'],
        errors: []
      };
    }

    const requiredFields = [
      { field: 'accountHolder', value: data.accountHolder, label: 'Nombre Titular' },
      { field: 'bank', value: data.bank, label: 'Banco' },
      { field: 'accountNumber', value: data.accountNumber, label: 'Número de Cuenta' },
      { field: 'transferRut', value: data.transferRut, label: 'RUT' },
      { field: 'confirmationEmail', value: data.confirmationEmail, label: 'Correo de Confirmación' }
    ];
    ...
    if (data.transferRut && data.transferRut.trim() !== '' && !validateRut(data.transferRut)) {
      errors.push({ field: 'transferRut', message: 'Formato de RUT inválido' });
    }

    if (data.confirmationEmail && data.confirmationEmail.trim() !== '' && !validateEmail(data.confirmationEmail)) {
      errors.push({ field: 'confirmationEmail', message: 'Formato de email inválido' });
    }
   ...
   */
  const loadTransferInfo = useCallback(async (forceRefresh = false) => {
    ...
    if (!forceRefresh) {
      ...
      if (cached) {
        ...
        setState(validation.isComplete ? TRANSFER_INFO_STATES.COMPLETE : TRANSFER_INFO_STATES.INCOMPLETE);
   ...
   */
  const isComplete = useMemo(() => state === TRANSFER_INFO_STATES.COMPLETE, [state]);
  const isLoading = useMemo(() => state === TRANSFER_INFO_STATES.LOADING, [state]);
  ...
  useEffect(() => {
    const checkInfo = async () => {
      ...
      try {
        ...
        if (!profileResponse || !profileResponse.data) {
          throw new Error('No se pudo obtener el perfil del usuario para verificación');
        }
        ...
        const requiredFields = ['accountHolder', 'bank', 'accountNumber', 'transferRut', 'confirmationEmail'];
        const missing = requiredFields.filter(field => !transferData[field] || transferData[field].trim() === '');
