...
export async function GetBuyerPaymentOrders(buyerId, { limit, offset } = {}) {
  const { data, error, count } = await ordersRepository.listByBuyer(buyerId, {
    limit,
    offset,
  })
  if (error) throw error
  if (!data || data.length === 0) return { orders: [], count: count || 0 }
  ...
  parsed.forEach(({ items }) =>
    items.forEach((it) => it?.supplier_id && supplierIdsSet.add(it.supplier_id))
  )
  ...
  if (productIdsSet.size) {
    const { data: products, error: prodErr } = await supabase
      .from('products')
      .select(
        'productid, product_images (image_url, thumbnail_url, thumbnails)'
      )
  ...
  const serviceObjects = parsed.map(({ row, items }) => {
    const normalizedItems = items.map((it, idx) => {
      ...
      const firstImage = Array.isArray(prod.product_images)
        ? prod.product_images[0]
        : {}
      const rawPriceAddition =
        it.unit_price_effective ?? it.price_at_addition ?? it.price
      let price_at_addition = 0
      let pricing_warning = false
      if (
        typeof rawPriceAddition === 'number' &&
        Number.isFinite(rawPriceAddition)
      ) {
        price_at_addition = rawPriceAddition
      } else if (typeof rawPriceAddition === 'string') {
        const cleaned = rawPriceAddition
          .trim()
          .replace(/[^0-9,\.]/g, '')
          .replace(',', '.')
        const parsed = Number(cleaned)
        if (Number.isFinite(parsed)) price_at_addition = parsed
        else pricing_warning = true
      } else {
        pricing_warning = true
      }
      if (!ALLOW_ZERO_PRICE_ITEMS && price_at_addition <= 0) {
        throw new Error(
          `Precio inválido (0) detectado en item ${idx} de orden ${row.id}`
        )
      }
      ...
      try {
        if (
          it.envio &&
          typeof it.envio === 'object' &&
          Number.isFinite(Number(it.envio.precio))
        ) {
          ...
          if (Array.isArray(arr) && arr.length) {
            const first = arr[0]
            if (first && Number.isFinite(Number(first.price)))
              shipping_price = Number(first.price)
          }
        }
      } catch (_) {}
      return {
        ...
        document_type: normalizeDocumentType(
          it.document_type || it.documentType
        ),
        pricing_warning,
        envio: it.envio || null,
    ...
    const computedLinesTotal = normalizedItems.reduce(
      (s, i) => s + i.price_at_addition * i.quantity,
      0
    )
    ...
    return {
      ...
      total_quantity: normalizedItems.reduce(
        (s, i) => s + (i.quantity || 0),
        0
      ),
  ...
  const uiOrders = domain.map((o) => toBuyerUIOrder(o))

  // Retornar objeto con orders y count para paginación
  return { orders: uiOrders, count: count || 0 }
}
