      ...
      {tramos.length >= 2 && (
        <Box sx={{ mt: 3 }}>
          ...
          {tramos.map((tramo, idx, arr) => {
            ...
            const max = nextTramo && nextTramo.cantidad ? nextTramo.cantidad - 1 : null;
            ...
            if (idx < arr.length - 1) {
              if (nextTramo && !nextTramo.cantidad) {
                return (
                  <Typography key={idx} variant="caption" color="text.secondary" display="block">
                    Tramo {idx + 1}: si el cliente compra entre <b>{min}</b> y <b>Cantidad a definir del Tramo {idx + 2}</b> unidades, paga <b>${precio}</b> por unidad.
                  </Typography>
                );
              } else {
                return (
                  <Typography key={idx} variant="caption" color="text.secondary" display="block">
                    Tramo {idx + 1}: si el cliente compra entre <b>{min}</b> y <b>{max}</b> unidades, paga <b>${precio}</b> por unidad.
                  </Typography>
                );
              }
            } else {
              ...
              if (tramo.cantidad) {
                return (
                  <Typography key={idx} variant="caption" color="text.secondary" display="block">
                    Tramo {idx + 1}: si compra <b>{min}</b> unidades o más, paga <b>${precio}</b> por unidad.
                  </Typography>
                );
              } else {
                return null;
