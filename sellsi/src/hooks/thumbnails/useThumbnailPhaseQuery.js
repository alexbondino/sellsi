...
function getPhaseQueryOptions(phase) {
  ...
  if (
    !phase ||
    phase === 'thumbnails_ready' ||
    phase === 'thumbnails_skipped_webp'
  ) {
    return {
      staleTime: 5 * 60 * 1000,
      gcTime: 15 * 60 * 1000,
...
async function fetchThumbnail(productId) {
  return scheduleThumbnailFetch(async () =>
    getOrFetchMainThumbnail(productId, { silent: true })
  );
}

export function useThumbnailPhaseQuery(
  productId,
  currentPhase,
  ...
  if (
    ...
    query.data &&
    phase &&
    !['thumbnails_ready', 'thumbnails_skipped_webp'].includes(phase)
  ) {
  ...
  if (
    query.data &&
    (phase === 'thumbnails_ready' || phase === 'thumbnails_skipped_webp')
  ) {
