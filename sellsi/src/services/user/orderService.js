...
class OrderService {
  ...
  */
  ...
  async updateOrderStatus(orderId, newStatus, additionalData = {}) {
    try {
      ...
      if (!orderError && orderData) {
        ...
        // Crear notificaciones para buyer (por cada item) según estado
        try { await this._notifyOrderStatusChange(orderData, normalizedStatus); } catch (_) {}
        return {
          success: true,
          order: orderData,
          source: 'orders',
  ...
  await this.logOrderAction(orderId, newStatus, additionalData);
  try { await this._notifyOrderStatusChange(cartData, normalizedStatus); } catch (_) {}
   ...
   * @private
   */
  async _notifyOrderStatusChange(orderRow, status) {
    if (!orderRow) return;
    const buyerId = orderRow.user_id || orderRow.buyer_id || orderRow.userId || null;
    if (!buyerId) return;

    // Obtener items desde orders (JSON) o carts (no los trae aquí; para carts se omite por simplicidad)
    let items = [];
    if (orderRow.items) {
      if (Array.isArray(orderRow.items)) items = orderRow.items;
      else if (typeof orderRow.items === 'string') { try { items = JSON.parse(orderRow.items); } catch (_) {} }
      else if (typeof orderRow.items === 'object') { items = Array.isArray(orderRow.items.items) ? orderRow.items.items : [orderRow.items]; }
    }
    if (!items.length) return; // sin items no notificamos por item
  ...
  async notifyNewOrder(orderRow) {
    ...
    // Notificación por item para buyer
    for (const it of items) {
      try {
        await supabase.rpc('create_notification', {
          p_user_id: buyerId,
    ...
    // Notificación para cada supplier (una por order) - agrupar por supplier_id
