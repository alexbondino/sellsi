...
jest.mock('../../../services/supabase', () => ({
  supabase: {
    ...
    single: jest.fn().mockResolvedValue({ data: { supplier_ids: ['sup-1','sup-2'] }, error: null }),
    auth: { getUser: jest.fn(), onAuthStateChange: () => ({ data: { subscription: { unsubscribe: () => {} } } }) },
...
import { useSupplierPartActions } from '../../../domains/supplier/hooks/useSupplierPartActions';
...
describe('useSupplierPartActions - routing mono vs multi supplier', () => {
  it('usa updateOrderStatus cuando supplier_ids.length === 1', async () => {
    const part = { order_id: 'o1', parent_order_id: 'o1', supplier_id: 'sup-1', supplier_ids: ['sup-1'] };
    const { result } = renderHook(() => useSupplierPartActions('sup-1'));
    await act(async () => { await result.current.accept(part); });
  ...
  it('usa updateSupplierPartStatus cuando supplier_ids.length > 1', async () => {
    const part = { order_id: 'o2', parent_order_id: 'o2', supplier_id: 'sup-1', supplier_ids: ['sup-1','sup-2'] };
    const { result } = renderHook(() => useSupplierPartActions('sup-1'));
    await act(async () => { await result.current.dispatch(part, '2025-12-31'); });
