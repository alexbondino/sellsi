...
jest.mock('../../shared/components/display/product-card/ProductCard', () => ({
  ...
  default: ({ product, onEdit, onDelete, onProductClick }) => (
    <div data-testid={`card-${product?.id || 'x'}`}>
      <button
        data-testid={`edit-${product?.id || 'x'}`}
        onClick={() => onEdit && onEdit(product)}
      >
        edit
      </button>
      <button
        data-testid={`delete-${product?.id || 'x'}`}
        onClick={() => onDelete && onDelete(product)}
      >
        delete
      </button>
      <button
        data-testid={`click-${product?.id || 'x'}`}
...
jest.mock('../../shared/components/feedback/AdvancedLoading', () => ({
  ...
  EmptyProductsState: () => <div data-testid="empty" />,
}));
  ...
  test('loads products when supplierId exists', async () => {
    ...
    console.warn = (...args) => {
      ...
      if (
        ...
        msg.includes('The `sm` prop has been removed') ||
        msg.includes('The `md` prop has been removed')
      )
        return;
      originalWarn(...args);
    };
    ...
    await waitFor(
      () => expect(loadProducts).toHaveBeenCalledWith('supplier-1'),
      { timeout: 3000 }
    );
    // restore
