...
jest.mock('@/domains/supplier/hooks/useProductForm', () => ({
  useProductForm: jest.fn(() => ({
    formData: { nombre: 'Test', pricingType: 'Unidad', tramos: [], stock: 10, imagenes: [{}, {}] },
    errors: {},
    touched: {},
    isLoading: false,
    isValid: true,
...
jest.mock('@/domains/supplier/pages/my-products/components', () => ({
  ...
  ProductResultsPanel: ({ onSubmit }) => (
    <div>
      <button data-testid="results-submit" onClick={onSubmit}>Submit (desktop)</button>
    </div>
  ),
  ProductPricing: () => <div data-testid="pricing" />,
  MobileExpandableBottomBar: ({ onSubmit }) => (
    <div>
      <button data-testid="mobile-submit" onClick={onSubmit}>Submit (mobile)</button>
    </div>
  ),
  PriceTiers: () => <div data-testid="pricetiers" />,
}));

jest.mock('@/domains/supplier/pages/my-products/hooks/useProductValidation', () => ({
...
jest.mock('@/domains/supplier/pages/my-products/hooks/useProductPricingLogic', () => ({
...
jest.mock('@/domains/supplier/hooks/useThumbnailStatus', () => ({
  useThumbnailStatus: () => ({
    isProcessing: false,
    isReady: false,
    hasError: false,
...
jest.mock('@/shared/services/supabase', () => ({
  default: { supabaseUrl: 'http://localhost', supabaseKey: 'key', supabase: { auth: { getUser: jest.fn(() => ({ data: { user: null } })) } } },
}));
...
describe('AddProduct - robust flows', () => {
  ...
  test('shows validation error and does not call submitForm when validation fails', async () => {
    ...
    jest.doMock('@/domains/supplier/pages/my-products/hooks/useProductValidation', () => ({
      useProductValidation: () => ({
        localErrors: {}, triedSubmit: false, validateForm: validateMock, resetErrors: jest.fn(), markSubmitAttempt: jest.fn()
      })
    }));

  // Build a small TestHost that reproduces only the submit flow using the mocked hooks.
  const TestHost = () => {
    const { formData, submitForm } = require('@/domains/supplier/hooks/useProductForm').useProductForm();
    const { validateForm, markSubmitAttempt } = require('@/domains/supplier/pages/my-products/hooks/useProductValidation').useProductValidation();
    ...
    const handleSubmit = async (e) => {
      ...
      if (result.success) {
        try { await marketplace.saveProductRegions(result.data.productid, []); } catch (err) { /* tolerate */ }
    ...
    return (
      <div>
        <button data-testid="results-submit" onClick={handleSubmit}>Submit (desktop)</button>
      </div>
    );
  };
  ...
  render(<TestHost />);
    ...
    const pf = require('@/domains/supplier/hooks/useProductForm').useProductForm();
  ...
  test('happy path: submit creates product, attempts to save regions and navigates after success, regions error is tolerated', async () => {
    ...
    jest.resetModules();
    const submitMock = jest.fn(() => Promise.resolve({ success: true, data: { productid: 'p-1' } }));
    jest.doMock('@/domains/supplier/hooks/useProductForm', () => ({
      useProductForm: () => ({
        formData: { nombre: 'Ok', pricingType: 'Unidad', tramos: [], stock: 1, imagenes: [{}, {}] },
        errors: {}, touched: {}, isLoading: false, isValid: true, hasActualChanges: false,
        updateField: jest.fn(), handleFieldBlur: jest.fn(), handlePricingTypeChange: jest.fn(), submitForm: submitMock, resetForm: jest.fn(), markImagesTouched: jest.fn()
      })
    }));

    jest.doMock('@/domains/supplier/pages/my-products/hooks/useProductValidation', () => ({
      useProductValidation: () => ({ localErrors: {}, triedSubmit: false, validateForm: jest.fn(() => ({})), resetErrors: jest.fn(), markSubmitAttempt: jest.fn() })
    }));
    ...
    jest.doMock('@/utils/toastHelpers', () => ({
      showSaveLoading: jest.fn(), replaceLoadingWithSuccess: jest.fn(), replaceLoadingWithError: jest.fn(),
    }));
  ...
  await waitFor(() => expect(marketplace.saveProductRegions).toHaveBeenCalled());
  ...
  await waitFor(() => expect(toastHelpers.replaceLoadingWithSuccess).toHaveBeenCalled());
  // Strict: replacement should be called with the product-save key and status OK
  expect(toastHelpers.replaceLoadingWithSuccess).toHaveBeenCalledWith('product-save', 'OK');
  ...
  const { useProductForm } = require('@/domains/supplier/hooks/useProductForm');
  const { formData } = useProductForm();
  const { useProductValidation } = require('@/domains/supplier/pages/my-products/hooks/useProductValidation');
  ...
  const handleSubmit = async () => {
    ...
    markSubmitAttempt();
    const errors = validateForm(formData);
    if (errors && Object.keys(errors).length > 0) { isSubmitting = false; return; }
    await submitMock();
    isSubmitting = false;
  };
