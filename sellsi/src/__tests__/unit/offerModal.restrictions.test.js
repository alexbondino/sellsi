...
describe('OfferModal restrictions', () => {
  ...
  it('bloquea envío si ya existe oferta pendiente para el producto', async () => {
    ...
    expect(
      await screen.findByTestId('pending-offer-block')
    ).toBeInTheDocument();
  ...
  it('muestra límite alcanzado por supplier_count (supplier_limit) aunque product_count < product_limit', async () => {
    ...
    const limitsNode = await screen.findByText(
      /límites de ofertas/i,
      {},
      { timeout: 3000 }
    );
    ...
    const reasonNodes = await screen.findAllByText(
      /proveedor/i,
      {},
      { timeout: 3000 }
    );
    expect(reasonNodes.length).toBeGreaterThan(0);
    const hasSupplierLimitReason = reasonNodes.some(n =>
      /límite mensual de ofertas \(proveedor\)/i.test(n.textContent)
    );
