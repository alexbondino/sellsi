...
import { useSupplierOffers } from '../../domains/supplier/pages/offers/hooks/useSupplierOffers';
...
describe('Offer Hooks', () => {
  ...
  describe('useSupplierOffers', () => {
    it('debería obtener ofertas del proveedor al montar', async () => {
      ...
      await waitFor(() => {
        expect(mockOfferStore.fetchSupplierOffers).toHaveBeenCalledWith('buyer_789');
      });
    ...
    it('debería sincronizar estado local con el store', async () => {
      ...
      const updatedOffers = [...initialOffers, { ...mockOfferData.validOffer, id: 'offer_456' }];
