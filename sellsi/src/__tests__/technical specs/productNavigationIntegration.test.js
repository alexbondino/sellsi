...
jest.mock('../../services/supabase', () => ({
  supabase: {
    ...
    storage: {
      from: jest.fn(() => ({
        getPublicUrl: jest.fn((image) => ({ data: { publicUrl: `/public/${image}` } }))
      }))
    }
  }
}))
...
describe('Product Navigation Integration Test', () => {
  ...
  it('should navigate from ProductCard to TechnicalSpecs without showing error flash', async () => {
    ...
    const productCard = screen.getByText('Laptop Gaming Test').closest('[role="button"], div[style*="cursor: pointer"], div')
  ...
  it('should show appropriate error message for missing products', async () => {
    ...
    window.history.pushState({}, '', '/technicalspecs/12345678-1234-1234-1234-123456789012-missing-product')
