...
export const useBuyerOrders = (buyerId) => {
  ...
  const [totalCount, setTotalCount] = useState(0) // Total de órdenes (para paginación)
  ...
  const mergeAndSplit = useCallback(
    (payment) => {
      // 1. Ordenar por fecha
      const ordered = payment.sort(
      ...
      for (const p of parts) {
        const key =
          p.parent_order_id + (p.supplier_id ? `-${p.supplier_id}` : '')
      ...
      if (DEBUG_BUYER_ORDERS)
        console.log('[useBuyerOrders] parts', result.length)
      return result
    },
    [DEBUG_BUYER_ORDERS]
  )
    ...
    async ({ limit, offset, ...filters } = {}) => {
      ...
      try {
        ...
        const result = await orderService
          .getPaymentOrdersForBuyer(buyerId, { limit, offset, ...filters })
          .catch((e) => {
            console.error('[buyerOrders][paymentOrders] error', e)
            setError(
              (prev) => prev || e.message || 'Error cargando payment orders'
            )
            return { orders: [], count: 0 }
          })

        // Extraer orders y count del resultado
        const payment = result.orders || []
        setTotalCount(result.count || 0)

        if (DEBUG_BUYER_ORDERS) {
          console.log(
            '[buyerOrders][raw payment orders]',
            payment.map((p) => ({
              id: p.order_id,
              status: p.status,
              eta: p.estimated_delivery_date,
            }))
          )
        }
        ...
        try {
          const orderIds = Array.from(
            new Set(payment.map((o) => o.order_id || o.id).filter(Boolean))
          )
          if (orderIds.length) {
          ...
          console.warn(
            '[buyerOrders] invoices_meta enrichment falló:',
            invE?.message || invE
          )
        }
        ...
        payment.forEach((o) => {
          ;(o.items || []).forEach((it) => {
            if (it.price_at_addition === 0 && it.quantity > 0) {
              ...
              console.warn('[buyerOrders][WARN] price_at_addition=0', {
                order_id: o.order_id,
                product_id: it.product_id,
                quantity: it.quantity,
                pricing_warning: it.pricing_warning,
              })
            }
          })
        })
        ...
        if (DEBUG_BUYER_ORDERS) {
          console.log(
            '[buyerOrders][after split]',
            split.map((p) => ({
              id: p.order_id,
              supplier: p.supplier_id,
              status: p.status,
              partEta: p.estimated_delivery_date,
            }))
          )
        }
        const enriched = split.map((part) => {
          try {
            ...
            if (!part.estimated_delivery_date) {
              ...
              if (base?.estimated_delivery_date)
                part.estimated_delivery_date = base.estimated_delivery_date
            }
            if (maybe?.path && Array.isArray(part.items)) {
              return {
                ...
                items: part.items.map((it) => ({
                  ...it,
                  invoice_path: it.invoice_path || maybe.path,
                })),
              }
            }
          } catch (_) {}
        ...
        if (split.length === 0 && !error) {
          ...
          console.warn(
            '[buyerOrders] Resultado vacío tras merge. Verificar flags, RLS o errores previos en consola.'
          )
        }
      } catch (err) {
  ...
  // NOTA: El componente BuyerOrders ahora maneja fetchOrders con paginación,
  // por lo que NO hacemos auto-fetch aquí para evitar doble carga
  const didFetchRef = useRef(false)
  const prevBuyerIdRef = useRef(buyerId)

  useEffect(() => {
    // If buyerId changed from null/invalid to valid, allow re-fetch
    const prevWasInvalid =
      !prevBuyerIdRef.current || !isUUID(prevBuyerIdRef.current)
    ...
    // DESHABILITADO: BuyerOrders.jsx maneja fetchOrders con paginación
  ...
  useEffect(() => {
    ...
    if (!ordersChannelRef.current) {
      ordersChannelRef.current = orderService.subscribeToBuyerPaymentOrders(
        buyerId,
        (payload) => {
          ...
          if (payload.eventType === 'UPDATE') {
            ...
            if (['paid', 'pending', 'expired'].includes(newPaymentStatus)) {
              setOrders((prev) => {
                ...
                // Nueva orden recién promovida a pagada pero no está en página actual:
                // NO hacer refetch automático (rompería paginación)
                // El usuario puede refrescar manualmente o cambiar de página
                return prev
              })
            }
          } else if (payload.eventType === 'INSERT') {
            // Nueva orden insertada: NO hacer refetch automático
            // En paginación backend, el INSERT podría no estar en la página actual
            // El usuario verá la nueva orden al navegar o refrescar
            // Opcional: emitir evento para mostrar notificación "Nueva orden disponible"
            if (DEBUG_BUYER_ORDERS) {
              console.log(
                '[buyerOrders][realtime] Nueva orden detectada (INSERT), no auto-refresh en paginación backend'
              )
            }
          }
        }
    ...
    if (!invoicesChannelRef.current) {
      invoicesChannelRef.current = supabase
        ...
        .on(
          'postgres_changes',
          { event: 'INSERT', schema: 'public', table: 'invoices_meta' },
          (payload) => {
            try {
              const row = payload.new
              ...
              setOrders((prev) =>
                prev.map((p) => {
                  ...
                  const already = p.items.some(
                    (it) => it.invoice_path === row.path
                  )
                  if (already) return p
                  return {
                    ...p,
                    items: p.items.map((it) => ({
                      ...it,
                      invoice_path: it.invoice_path || row.path,
                    })),
                  }
                })
              )
        ...
        if (Array.isArray(statuses)) {
          setOrders((prev) =>
            prev.map((o) => {
              ...
              return st
                ? { ...o, payment_status: st.payment_status, status: o.status }
                : o
            })
          )
        }
    ...
    try {
      ...
      setOrders((prev) => {
        const filtered = prev.filter((o) => {
          ...
          const orderIdMatch =
            o.order_id === orderId || o.parent_order_id === orderId
          return !orderIdMatch
        })

        // Decrementar totalCount si se eliminó una orden
        // Contar cuántas órdenes únicas se eliminaron (por parent_order_id)
        const removedOrderIds = new Set()
        prev.forEach((o) => {
          const orderMatch =
            o.order_id === orderId || o.parent_order_id === orderId
          if (orderMatch) {
            removedOrderIds.add(o.parent_order_id || o.order_id)
          }
        })

        if (removedOrderIds.size > 0) {
          setTotalCount((prevCount) =>
            Math.max(0, prevCount - removedOrderIds.size)
          )
        }
