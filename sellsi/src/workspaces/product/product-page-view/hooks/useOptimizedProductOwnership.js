...
const fetchUserDataCentralized = async () => {
    ...
    return new Promise(resolve => {
      const checkInterval = setInterval(() => {
        if (!globalOwnershipCache.isLoading) {
          ...
          globalOwnershipCache.metrics.averageVerificationTime =
            endTime - startTime;
  ...
  if (
    ...
    Date.now() - globalOwnershipCache.timestamp <
      globalOwnershipCache.CACHE_DURATION
  ) {
    const endTime = performance.now();
  ...
  try {
    ...
    if (error) {
      ...
      globalOwnershipCache.timestamp =
        Date.now() - (globalOwnershipCache.CACHE_DURATION - 30000); // 30s antes de expirar
    } else {
      globalOwnershipCache.userId = currentUserId;
...
export const useOptimizedProductOwnership = () => {
  ...
  const [isLoadingOwnership, setIsLoadingOwnership] = useState(
    globalOwnershipCache.isLoading
  );
   ...
   */
  const isProductOwnedByUser = useCallback(
    product => {
      const startTime = performance.now();
   ...
   */
  const getProductsOwnership = useCallback(
    products => {
      const startTime = performance.now();
