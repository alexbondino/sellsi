...
async function applyMlOrdering(products) {
  try {
    const recommendationsData = await getRecommendations(
      Math.min(products.length, 50)
    );

    if (recommendationsData && Array.isArray(recommendationsData.products)) {
...
export function useProducts() {
  ...
  const refreshProducts = useCallback(async controller => {
    ...
    const finalProducts = await applyMlOrdering(enriched);
