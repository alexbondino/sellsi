...
const useProductCleanup = create((set, get) => ({
  ...
  cleanupOrphanedFiles: async supplierId => {
    ...
    try {
      ...
      const orphanedFiles = await get().findOrphanedFiles(
        supplierId,
        registeredUrls
      );
  ...
  findOrphanedFiles: async (supplierId, registeredUrls) => {
    ...
    try {
      ...
      const mainBucketFiles = await get().scanBucket(
        'product-images',
        `${supplierId}/`,
        registeredUrls
      );
      ...
      const thumbnailBucketFiles = await get().scanBucket(
  ...
  scanBucket: async (bucketName, prefix, registeredUrls) => {
    ...
    try {
      ...
      for (const subdir of subdirs) {
        const subResult = await get().scanBucket(
          bucketName,
          `${prefix}${subdir.name}/`,
          registeredUrls
        );
      ...
      throw new Error(
        `Error escaneando bucket ${bucketName}: ${error.message}`
      );
    }
  },
  ...
  cleanupOrphanedRecords: async supplierId => {
    ...
    try {
      ...
      for (const record of imageRecords) {
        ...
        if (record.image_url) {
          const fileExists = await get().verifyFileExists(
            record.image_url,
            'product-images'
          );
        ...
        if (record.thumbnail_url && !shouldDelete) {
          const thumbnailExists = await get().verifyFileExists(
            record.thumbnail_url,
            'product-images-thumbnails'
          );
          // Solo eliminar si ambos archivos no existen
          if (!thumbnailExists && record.image_url) {
            const mainExists = await get().verifyFileExists(
              record.image_url,
              'product-images'
            );
  ...
  auditDataConsistency: async supplierId => {
    try {
      ...
      for (const image of images) {
        ...
        const parentExists = products.some(
          p => p.productid === image.product_id
        );
