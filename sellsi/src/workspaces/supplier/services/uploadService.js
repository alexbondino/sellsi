...
export class UploadService {
   ...
   * @returns {Promise<{success: boolean, data?: any[], errors?: string[]}>}
   */
  static async uploadMultipleImagesWithThumbnails(
    files,
    productId,
    supplierId,
    options = {}
    ...
    try {
      ...
      if (replaceExisting) {
        try {
          const cleanupResult =
            await StorageCleanupService.deleteAllProductImages(productId);
        } catch (cleanupError) {
          console.warn(
            '⚠️ [uploadMultipleImages] Error en limpieza (continuando):',
      ...
      if (replaceExisting) {
        const existingFiles = files.filter(
          file => file.isExisting || (file.file && file.file.size === 0)
        );
        ...
        const referencePromises = existingFiles.map(async (file, index) => {
          if (file.url) {
            ...
            if (dbInsertError) {
              console.error(
                '❌ [uploadMultipleImages] Error recreando referencia:',
        ...
        if (filesToProcess.length > 0) {
          ...
          const firstRes = await this.uploadImageWithThumbnail(
            filesToProcess[0],
            productId,
            supplierId,
            true
        ...
        if (filesToProcess.length > 1) {
          const parallel = await Promise.allSettled(
            filesToProcess
              .slice(1)
              .map(f =>
                this.uploadImageWithThumbnail(f, productId, supplierId, false)
              )
          );
        ...
        results.forEach((result, idx) => {
          if (result.status === 'fulfilled' && result.value?.success) {
            ...
            errors.push(
              `Archivo ${fileRef?.name || fileRef?.file?.name}: ${errorMsg}`
            );
          }
        });
        ...
        if (successful.length > 0) {
          this.dispatchProductImagesReady(productId, {
            ...
            mode: 'multiple',
            mainUpdated: successful.some(img => img.isMain),
          });
        }
        return {
      ...
      const newFiles = files.filter(file => {
        ...
        const isExisting =
          file.isExisting || (file.file && file.file.size === 0);
        return !isExisting;
      });
   ...
   */
  static async replaceAllProductImages(
    files,
    productId,
    supplierId,
    opts = {}
    ...
    try {
      if (!files || files.length === 0) {
        ...
        this._dispatchPhase(productId, 'base_insert', {
          count: 0,
          mode: 'replace',
        });
      ...
      for (let idx = 0; idx < files.length; idx++) {
        ...
        const isExisting =
          item?.isExisting === true || (!!item?.url && !item?.file);
        if (isExisting) {
          const existingUrl = item.url || item.image_url || item.publicUrl;
          if (!existingUrl) {
            console.warn(
              '⚠️ [replaceAllProductImages] Entrada marcada existing sin URL, se ignora índice',
              idx
            );
            continue;
          }
          ...
          if (!actualFile || !actualFile.name || !actualFile.type) {
            console.error(
              '❌ [replaceAllProductImages] Entrada no válida para upload en índice',
              idx,
              item
            );
            return {
              success: false,
              error:
                'Entrada de imagen no válida (faltan metadatos de archivo)',
            };
          }
          ...
          if (!uploadRes.success) {
            console.error(
              '❌ [replaceAllProductImages] Falló upload índice',
              idx,
              uploadRes.error
            );
      ...
      if (!expectedMainEntry) {
        return {
          success: false,
          error:
            'No se encontró ninguna entrada de imagen válida (todas inválidas)',
        };
      }
      ...
      const rpcName = FeatureFlags.ENABLE_SIGNATURE_COLUMN
        ? 'replace_product_images_preserve_thumbs'
        : 'replace_product_images';
      const { data: replacedRows, error: replaceErr } = await supabase.rpc(
        rpcName,
        {
      ...
      try {
        ...
        if (!reErr && Array.isArray(recheck)) {
          ...
          if (!sequenceOk) {
            console.error(
              '❌ [replaceAllProductImages] Secuencia image_order inconsistente',
              recheck
            );
          }
          ...
          if (recheck.length !== orderedUrls.length) {
            console.error(
              '❌ [replaceAllProductImages] Cardinalidad inesperada',
              { esperado: orderedUrls.length, real: recheck.length }
            );
          }
        }
      } catch (vErr) {
        console.warn(
          '⚠️ [replaceAllProductImages] Verificación post-replace falló:',
          vErr.message
        );
      }
      ...
      try {
        if (
          ...
          verifiedRows[0].image_url !== expectedMainUrl
        ) {
          const currentMain = verifiedRows[0];
          const expectedIdx = verifiedRows.findIndex(
            r => r.image_url === expectedMainUrl
          );
          if (expectedIdx > -1) {
            const expectedRow = verifiedRows[expectedIdx];
            thumbsLog('MAIN_SWAP', {
              currentMain: currentMain.image_url,
              expectedMain: expectedMainUrl,
              expectedIdx,
            });
            // Swap en 3 pasos para respetar índice único (image_order=0)
            await supabase
              .from('product_images')
              .update({ image_order: -1 })
              .eq('id', expectedRow.id);
            await supabase
              .from('product_images')
              .update({ image_order: expectedRow.image_order })
        ...
        console.error(
          '❌ [replaceAllProductImages] Error corrigiendo main:',
          swapErr.message
        );
      }
      ...
      if ((verifiedRows?.length || 0) > 0) {
        this._ensureMainThumbnails(
          productId,
          supplierId,
          verifiedRows[0].image_url
        )
          .then(result => {
            // Propagar posible staleDetected (si Edge enforcement aplicado) en evento final
            if (result.status === 'ready')
              this._dispatchPhase(productId, 'thumbnails_ready', result);
            else if (result.status === 'skipped_webp')
              this._dispatchPhase(productId, 'thumbnails_skipped_webp', result);
            else if (result.status === 'partial')
              this._dispatchPhase(productId, 'thumbnails_partial', result);
            else this._dispatchPhase(productId, 'thumbnails_failed', result);
            if (['ready', 'partial'].includes(result.status)) {
              setTimeout(
                () => this._autoRepairIf404(productId, supplierId),
                1500
              );
              // Iniciar grace period para cleanup (45s)
              try {
                StorageCleanupService.markRecentGeneration(productId, 45000);
              } catch (_) {}
            }
          })
          .catch(err => {
            recordMetric('generation_result', {
              productId,
              outcome: 'exception',
              error: err?.message,
            });
            this._dispatchPhase(productId, 'thumbnails_failed', {
              error: err?.message,
            });
          });
      }
      ...
      if (cleanup) {
        ...
        if (FeatureFlags.ENABLE_DELAYED_CLEANUP) {
          ...
          const listener = ev => {
            if (
              ev?.detail?.productId === productId &&
              [
                'thumbnails_ready',
                'thumbnails_skipped_webp',
                'thumbnails_partial',
                'thumbnails_failed',
              ].includes(ev?.detail?.phase)
            ) {
          ...
          try {
            window.addEventListener('productImagesReady', listener);
          } catch (_) {
            /* noop */
          }
      ...
      try {
        console.log(
          `🔥 Triggering cache invalidation for product ${productId} after image replacement`
        );
        ...
        if (window.thumbnailInvalidationService) {
          window.thumbnailInvalidationService.invalidateProductThumbnails(
            productId
          );
        }
      } catch (invalidationError) {
        console.warn(
          '⚠️ Cache invalidation failed (non-critical):',
          invalidationError
        );
      }
  ...
  static async uploadImageWithThumbnail(
    ...
    try {
      ...
      if (!actualFile) {
        console.error(
          '❌ [uploadImageWithThumbnail] No se proporcionó archivo'
        );
        return { success: false, error: 'No se proporcionó archivo' };
      }
      ...
      if (!actualFile.type || !actualFile.type.startsWith('image/')) {
        console.error(
          '❌ [uploadImageWithThumbnail] Tipo de archivo inválido:',
          actualFile.type
        );
        return { success: false, error: 'Solo se permiten archivos de imagen' };
      }
      ...
      if (actualFile.size > this.MAX_IMAGE_SIZE) {
        console.error(
          '❌ [uploadImageWithThumbnail] Archivo muy grande:',
          actualFile.size,
          'vs',
          this.MAX_IMAGE_SIZE
        );
      ...
      try {
        ...
        if (rpcError) {
          console.error(
            '❌ [uploadImageWithThumbnail] Error RPC insert_image_with_order:',
            rpcError.message
          );
        } else {
          ...
          try {
            ...
            if (thumbnailResult.success) {
              ...
              if (thumbnailResult.wasRetried) {
                console.info(
                  `✅ [uploadImageWithThumbnail] Thumbnail generado exitosamente después de ${thumbnailResult.attemptUsed} intentos`
                );
              }
            } else {
              console.warn(
                `⚠️ [uploadImageWithThumbnail] Falló generación de thumbnail después de ${thumbnailResult.attemptUsed} intentos:`,
                thumbnailResult.error
              );
            }
          } catch (thumbnailError) {
            console.error(
              '❌ [uploadImageWithThumbnail] Error crítico en generación de thumbnail:',
              thumbnailError
            );
      ...
      return {
        success: false,
        error: `Error al subir imagen ${actualFile?.name}: ${
   ...
   */
  static async generateThumbnailWithRetry(
    imageUrl,
    productId,
    supplierId,
    options = {}
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries) {
          ...
          console.warn(
            `🔄 [generateThumbnailWithRetry] Intento ${attempt} falló, reintentando en ${delay}ms...`
          );
          await new Promise(resolve => setTimeout(resolve, delay));
        }
      } catch (error) {
        console.error(
          `❌ [generateThumbnailWithRetry] Error en intento ${attempt}:`,
          error.message
        );
        if (attempt === maxRetries) {
          return {
  ...
  static async generateThumbnail(
    ...
    try {
      const response = await fetch(
        `${supabase.supabaseUrl}/functions/v1/generate-thumbnail`,
        {
          method: 'POST',
          headers: {
  ...
  static async _autoRepairIf404(productId, supplierId) {
    try {
      ...
      let status = 0;
      try {
        const resp = await fetch(row.thumbnail_url, {
          method: 'HEAD',
      ...
      clearTimeout(t);
      if (status === 404) {
        recordMetric('generation_start', {
          productId,
          reason: 'auto_repair_404',
        });
        this._dispatchPhase(productId, 'repair', {
          reason: 'thumbnail_404_detected',
        });
        // 🔥 MEJORA: Forzar regeneración con retry logic para auto-repair
        const regen = await this.generateThumbnailWithRetry(
          row.image_url,
          productId,
          supplierId,
          { force: true, maxRetries: 2 }
        );
        recordMetric('generation_result', {
          productId,
          outcome: regen?.success
            ? 'repair_forced_success'
            : 'repair_forced_failed',
        });
      }
    } catch (_) {
      /* noop */
      ...
      for (let attempt = 1; attempt <= BACKOFFS.length; attempt++) {
        ...
        if (hasAll) {
          ...
          return {
            ...
            status: 'ready',
            attempt,
            signature: mainRow?.thumbnail_signature || null,
          };
        }
        const hasDesktop = !!(
          mainRow &&
        ...
        if (!hasDesktop) {
          ...
          const gen = await this.generateThumbnailWithRetry(
            mainImageUrl,
            productId,
            supplierId,
            { maxRetries: 1 }
      ...
      if (lastRow && lastRow.thumbnail_url && lastRow.thumbnails) {
        ...
        if (haveSome && !complete) {
          ...
          return {
            ...resultBase,
            status: 'partial',
            signature: lastRow?.thumbnail_signature || null,
