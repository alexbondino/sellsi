...
import {
  ...
  ERROR_MESSAGES,
  VALIDATION_UTILS,
} from './productValidationConstants';
...
export class ProductValidator {
  ...
  static _validateBasicInfo(formData, errors) {
    ...
    if (!formData.descripcion?.trim()) {
      ...
      formData.descripcion.length < PRODUCT_LIMITS.MIN_DESCRIPTION_LENGTH
    ) {
      errors.descripcion = ERROR_MESSAGES.DESCRIPTION_TOO_SHORT;
    } else if (
      formData.descripcion.length > PRODUCT_LIMITS.MAX_DESCRIPTION_LENGTH
    ) {
  ...
  static _validateInventory(formData, errors) {
    ...
    if (formData.pricingType !== PRICING_TYPES.TIER) {
      ...
      if (!formData.compraMinima) {
        ...
        if (
          isNaN(compraMinima) ||
          compraMinima < QUANTITY_LIMITS.MIN_QUANTITY
        ) {
  ...
  static _validateImages(formData, errors) {
    if (!formData.imagenes?.length) {
      ...
      const oversizedImages = formData.imagenes.filter(
        img =>
          img.file &&
          img.file.size > PRODUCT_LIMITS.MAX_IMAGE_SIZE_MB * 1024 * 1024
      );
  ...
  static generateContextualMessage(validationErrors) {
    console.log(
      '🔍 [ProductValidator.generateContextualMessage] Procesando errores:',
      validationErrors
    );
    ...
    console.log(
      '🔍 [ProductValidator.generateContextualMessage] Tipos de errores detectados:',
      {
        hasTramoErrors,
        hasBasicFieldErrors,
        hasImageErrors,
    ...
    if (hasTramoErrors) {
      ...
      if (tramoError.includes('ascendentes')) {
        messages.push(
          '🔢 Las cantidades de los tramos deben ser ascendentes (ej: 50, 100, 200)'
        );
      } else if (
        tramoError.includes('descendentes') ||
        tramoError.includes('compran más')
      ) {
        messages.push(
          '💰 Los precios deben ser descendentes: compran más, pagan menos por unidad'
        );
        ...
        messages.push(
          '📈 Debes configurar al menos 2 tramos de precios válidos'
        );
      } else if (tramoError.includes('stock')) {
        messages.push(
          '⚠️ Las cantidades de los tramos no pueden superar el stock disponible'
        );
      } else if (tramoError.includes('enteros positivos')) {
        messages.push(
          '🔢 Las cantidades y precios deben ser números enteros positivos'
        );
      } else {
    ...
    if (messages.length > 1) {
      ...
      console.log(
        '📝 [ProductValidator.generateContextualMessage] Mensaje múltiple:',
        finalMessage
      );
      return finalMessage;
    } else if (messages.length === 1) {
      console.log(
        '📝 [ProductValidator.generateContextualMessage] Mensaje único:',
        messages[0]
      );
      return messages[0];
    } else {
      ...
      console.log(
        '📝 [ProductValidator.generateContextualMessage] Mensaje por defecto:',
        defaultMessage
      );
