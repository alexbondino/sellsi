...
import {
  ...
  renamePreset,
  deletePreset,
} from '../../../../services/supplier/shippingRegionPresetsService';
...
export function useShippingRegionPresets(supplierId) {
  ...
  const savePreset = useCallback(
    async (index, name, regionsDisplay) => {
      setSaving(true);
      setError(null);
      try {
  ...
  const rename = useCallback(
    async (index, newName) => {
      ...
      setError(null);
      try {
        await renamePreset(supplierId, index, newName);
        await load();
      } catch (e) {
        setError(e.message);
      } finally {
  ...
  const remove = useCallback(
    async index => {
      ...
      setError(null);
      try {
        await deletePreset(supplierId, index);
        await load();
      } catch (e) {
        setError(e.message);
      } finally {
  ...
  const getPresetByIndex = useCallback(
    i => presets.find(p => p.index === i) || null,
    [presets]
  );
