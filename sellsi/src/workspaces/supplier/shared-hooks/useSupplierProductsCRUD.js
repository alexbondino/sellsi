...
const useSupplierProductsCRUD = create((set, get) => ({
  ...
  loadProducts: async supplierId => {
    ...
    try {
      ...
      const allEmpty =
        ...
        processedProducts.every(
          p => !p.priceTiers || p.priceTiers.length === 0
        );
      if (allEmpty) {
  ...
  createBasicProduct: async productData => {
    ...
    try {
      ...
      const newProduct = {
        ...
        minPrice: (() => {
          const arr = Array.isArray(productData.priceTiers)
            ? productData.priceTiers
            : [];
          const nums = arr
            .map(t => Number(t.price ?? t.precio ?? 0))
            .filter(n => n > 0);
  ...
  updateBasicProduct: async (productId, updates) => {
    ...
    try {
      ...
      const { priceTiers, imagenes, specifications, ...productFields } =
        updates;
  ...
  refreshProduct: async productId => {
    try {
      ...
      set(state => {
        ...
        const hasDbTiers =
          Array.isArray(processedProduct.priceTiers) &&
          processedProduct.priceTiers.length > 0;
        const mergedPriceTiers = hasDbTiers
          ? processedProduct.priceTiers
          : oldProduct.priceTiers || [];
        const mergedTiersStatus = hasDbTiers
          ? 'loaded'
          : oldProduct.tiersStatus || 'idle';
        const mergedProduct = {
          ...processedProduct,
          priceTiers: mergedPriceTiers,
          tiersStatus: mergedTiersStatus,
        };
