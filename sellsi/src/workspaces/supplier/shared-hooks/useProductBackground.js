...
const useProductBackground = create((set, get) => ({
  ...
  processProductInBackground: async (productId, productData, hooks = {}) => {
    const { imagesHook, specificationsHook, priceTiersHook, crudHook } = hooks;
      ...
      backgroundTasks: {
        ...
        [productId]: {
          ...
          operations: {
            ...
            specifications:
              productData.specifications?.length > 0 ? 'pending' : 'skipped',
            priceTiers:
              productData.priceTiers?.length > 0 ? 'pending' : 'skipped',
            reload: 'pending',
          },
        },
      },
    ...
    try {
      ...
      if (
        productData.imagenes !== undefined &&
        imagesHook &&
        typeof imagesHook.uploadImages === 'function'
      ) {
        updateProgress('images', 'processing');

        const supplierId =
          productData.supplier_id || localStorage.getItem('user_id');
        ...
        if (!result.success) {
          const errorMsg =
            result.error ||
            result.errors?.join(', ') ||
            'Error desconocido en procesamiento de imágenes';
        ...
        if (result.success && crudHook && crudHook.refreshProduct) {
          ...
          window.dispatchEvent(
            new CustomEvent('productImagesReady', {
              detail: {
                productId,
          ...
          if (!FeatureFlags.ENABLE_PHASED_THUMB_EVENTS) {
            setTimeout(async () => {
              await crudHook.refreshProduct(productId);
            }, 100);
          }
        }
      }
      ...
      if (productData.specifications?.length > 0 && specificationsHook) {
        ...
        const result = await specificationsHook.processProductSpecifications(
          productId,
        ...
        if (!result.success) {
          const errorMsg =
            result.error ||
            result.errors?.join(', ') ||
            'Error desconocido en procesamiento de especificaciones';
      ...
      if (productData.priceTiers?.length > 0 && priceTiersHook) {
        ...
        const result = await priceTiersHook.processPriceTiers(
          productId,
        ...
        if (!result.success) {
          const errorMsg =
            result.error ||
            result.errors?.join(', ') ||
            'Error desconocido en procesamiento de tramos de precio';
    ...
    try {
      ...
      if (!createResult.success) {
        const errorMsg =
          ...
          'Error desconocido creando producto';
      ...
      if (
        ...
        get()
          .processProductInBackground(productId, productData, hooks)
          .catch(error => {
            console.error(
              '🔥 [createCompleteProduct] Error crítico en background processing:',
              {
                productId,
                error: error.message,
                stack: error.stack,
            ...
            if (typeof window !== 'undefined') {
              window.dispatchEvent(
                new CustomEvent('productBackgroundError', {
  ...
  updateCompleteProduct: async (productId, updates, hooks = {}) => {
    ...
    try {
      ...
      if (crudHook) {
        const updateResult = await crudHook.updateBasicProduct(
          productId,
          updates
        );
        ...
        if (!updateResult.success) {
          const errorMsg =
            ...
            'Error desconocido actualizando producto';
          throw new Error(errorMsg);
        }
      }
      ...
      if (updates.priceTiers !== undefined && priceTiersHook) {
        ...
        try {
          if (crudHook && typeof crudHook.updateLocalProduct === 'function') {
            ...
            crudHook.updateLocalProduct(productId, {
        ...
        const priceTierResult = await priceTiersHook.processPriceTiers(
          productId,
          updates.priceTiers
        );
        ...
        if (!priceTierResult.success) {
          const errorMsg =
            priceTierResult.error ||
            priceTierResult.errors?.join(', ') ||
            'Error desconocido procesando tramos de precio';
  ...
  retryFailedTask: async (productId, productData, hooks = {}) => {
    ...
    return await get().processProductInBackground(
      productId,
      productData,
      hooks
    );
  ...
  hasActiveTasks: () => {
    const state = get();
    return Object.values(state.backgroundTasks).some(
      task =>
