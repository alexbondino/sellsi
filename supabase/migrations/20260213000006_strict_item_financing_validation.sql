-- 20260213000006_strict_item_financing_validation.sql
-- BUGFIX: evitar consumo parcial silencioso cuando hay múltiples financing_request_id por ítem.
-- En vez de omitir grupos inválidos con WARNING, aborta la operación para mantener atomicidad.
...
BEGIN
  ...
  AND NEW.financing_amount > 0 THEN
    ...
    IF v_buyer_id IS NULL THEN
      RAISE EXCEPTION 'BUYER_NOT_FOUND_FOR_ORDER user_id=% order_id=%', NEW.user_id, NEW.id;
    END IF;
    ...
    IF v_has_item_financing_ids THEN
      ...
      LOOP
        ...
        EXCEPTION WHEN others THEN
          RAISE EXCEPTION 'INVALID_FINANCING_REQUEST_ID value=% order_id=%', v_financing_id_text, NEW.id;
        END;
        ...
        IF NOT FOUND THEN
          RAISE EXCEPTION 'FINANCING_REQUEST_NOT_VALID financing_id=% buyer_id=% order_id=%',
            v_financing_id, v_buyer_id, NEW.id;
        END IF;

        IF v_group_financing_amount > GREATEST(0, COALESCE(v_financing_request.available_amount, 0)) THEN
          RAISE EXCEPTION 'FINANCING_AMOUNT_EXCEEDS_AVAILABLE financing_id=% amount=% available=% order_id=%',
            v_financing_id, v_group_financing_amount, v_financing_request.available_amount, NEW.id;
        END IF;
        ...
        IF v_first_supplier_user_id IS NULL THEN
          RAISE EXCEPTION 'ITEM_SUPPLIER_ID_MISSING financing_id=% order_id=%', v_financing_id, NEW.id;
        END IF;
        ...
        IF v_supplier_id IS NULL OR v_supplier_id <> v_financing_request.supplier_id THEN
          RAISE EXCEPTION 'FINANCING_SUPPLIER_MISMATCH financing_id=% item_supplier_id=% financing_supplier_id=% order_id=%',
            v_financing_id, v_supplier_id, v_financing_request.supplier_id, NEW.id;
        END IF;
