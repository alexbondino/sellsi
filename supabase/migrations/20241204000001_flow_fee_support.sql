-- Migration: Flow Fee Support in finalize_order_pricing
-- Date: 2024-12-04
-- Description: Extend finalize_order_pricing to calculate 3.19% fee for Flow payments

-- Replace function to add Flow fee calculation (3.19% of total base)
...
BEGIN
  ...
  ELSIF v_order.payment_method = 'flow' THEN
    -- Flow: comisión de 3.19% sobre el total base (subtotal + shipping)
    v_payment_fee := ROUND(v_total_base * 0.0319);
  ELSE
    v_payment_fee := 0;
  END IF;
...
COMMENT ON FUNCTION public.finalize_order_pricing(uuid) IS 'Seal pricing (gross): tiers, offers, derive included tax, add fee (Khipu $500 / Flow 3.19%), hash.';
