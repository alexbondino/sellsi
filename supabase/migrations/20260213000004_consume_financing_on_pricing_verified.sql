-- 20260213000004_consume_financing_on_pricing_verified.sql
-- Objetivo: consumir financiamiento al sellar pricing (pricing_verified_at),
-- incluso si payment_status sigue 'pending' en órdenes mixtas.
-- Esto alinea la UX de items 100% financiados con el estado contable (amount_used).
...
BEGIN
  IF (
    ...
    OR (
      NEW.payment_status IN ('pending', 'paid')
      AND OLD.pricing_verified_at IS NULL
      AND NEW.pricing_verified_at IS NOT NULL
    )
  )
...
CREATE TRIGGER trg_create_financing_transaction_on_payment
  ...
  FOR EACH ROW
  WHEN (
    NEW.financing_amount > 0
    AND (
      (NEW.payment_status = 'paid' AND (OLD.payment_status IS NULL OR OLD.payment_status != 'paid'))
      OR
      (NEW.payment_status IN ('pending', 'paid') AND OLD.pricing_verified_at IS NULL AND NEW.pricing_verified_at IS NOT NULL)
    )
  )
  EXECUTE FUNCTION public.create_financing_transaction_on_payment();

CREATE OR REPLACE FUNCTION public.trigger_rollback_financing_on_order_failure_status()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  v_result jsonb;
BEGIN
  IF COALESCE(NEW.financing_amount, 0) <= 0 THEN
    RETURN NEW;
  END IF;

  IF NEW.payment_status IN ('failed', 'rejected', 'expired', 'cancelled')
     AND COALESCE(OLD.payment_status, '') NOT IN ('failed', 'rejected', 'expired', 'cancelled') THEN

    SELECT public.rollback_order_financing_on_payment_failure(
      NEW.id,
    ...
    IF v_result ? 'error' THEN
      RAISE WARNING '[trigger_rollback_financing_on_order_failure_status] rollback error for order %: %',
        NEW.id,
        v_result->>'error';
    END IF;
  END IF;
...
DROP TRIGGER IF EXISTS trg_rollback_financing_on_order_failure_status ON public.orders;

CREATE TRIGGER trg_rollback_financing_on_order_failure_status
  AFTER UPDATE OF payment_status ON public.orders
  FOR EACH ROW
  WHEN (
    NEW.financing_amount > 0
    AND NEW.payment_status IN ('failed', 'rejected', 'expired', 'cancelled')
    AND (OLD.payment_status IS DISTINCT FROM NEW.payment_status)
  )
  EXECUTE FUNCTION public.trigger_rollback_financing_on_order_failure_status();
