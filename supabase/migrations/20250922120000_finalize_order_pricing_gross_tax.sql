-- Migration: finalize_order_pricing uses gross pricing (IVA incluido)
-- Date: 2025-09-22

-- Replace function to derive included IVA from subtotal and avoid re-adding it to total
...
DECLARE
  ...
  v_payment_fee numeric := 0;     -- khipu flat fee (placeholder)
...
BEGIN
  ...
  -- Payment fee (Khipu) según método ya seteado en la orden
...
COMMENT ON FUNCTION public.finalize_order_pricing(uuid) IS 'Seal pricing (gross): tiers, offers, derive included tax, add fee, hash.';
