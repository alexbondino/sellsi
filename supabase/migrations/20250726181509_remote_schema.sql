...
BEGIN
  ...
  INSERT INTO public.users (user_id, email, user_nm, main_supplier, phone_nbr, country, createdt, updatedt)
  VALUES (NEW.id, NEW.email, COALESCE(meta->>'full_name', 'Pendiente'), COALESCE((meta->>'proveedor')::BOOLEAN, FALSE), meta->>'phone', COALESCE(meta->>'pais', 'CL'), NOW(), NOW());
  RETURN NEW;
END;$$;
...
BEGIN
  ...
  SET
    email = NEW.email,
    user_nm = COALESCE(meta->>'full_name', user_nm),
    main_supplier = COALESCE((meta->>'proveedor')::BOOLEAN, main_supplier),
    phone_nbr = COALESCE(meta->>'phone', phone_nbr),
...
CREATE TABLE IF NOT EXISTS "public"."admin_audit_log" ("id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL, "admin_id" "uuid" NOT NULL, "action" "text" NOT NULL, "target_id" "uuid", "details" "jsonb", "timestamp" timestamp with time zone DEFAULT "now"(), "ip_address" "text", "user_agent" "text");
ALTER TABLE "public"."admin_audit_log" OWNER TO "postgres";

CREATE TABLE IF NOT EXISTS "public"."admin_sessions" ("session_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL, "admin_id" "uuid" NOT NULL, "created_at" timestamp with time zone DEFAULT "now"(), "expires_at" timestamp with time zone, "ip_address" "text", "user_agent" "text", "is_active" boolean DEFAULT true);
ALTER TABLE "public"."admin_sessions" OWNER TO "postgres";

CREATE TABLE IF NOT EXISTS "public"."bank_info" ("user_id" "uuid", "account_holder" character varying, "bank" character varying, "account_number" character varying, "transfer_rut" character varying, "confirmation_email" "text", "account_type" character varying DEFAULT 'corriente'::character varying);
ALTER TABLE "public"."bank_info" OWNER TO "postgres";

CREATE TABLE IF NOT EXISTS "public"."banned_ips" ("ip" "text" NOT NULL, "banned_at" timestamp with time zone DEFAULT "now"(), "banned_reason" "text", "banned_by" "uuid");
ALTER TABLE "public"."banned_ips" OWNER TO "postgres";

CREATE TABLE IF NOT EXISTS "public"."billing_info" ("user_id" "uuid", "business_name" character varying, "billing_rut" character varying, "business_line" character varying, "billing_address" "text", "billing_region" "text", "billing_commune" "text");
ALTER TABLE "public"."billing_info" OWNER TO "postgres";

CREATE TABLE IF NOT EXISTS "public"."cart_items" ("cart_id" "uuid" NOT NULL, "product_id" "uuid" NOT NULL, "quantity" integer NOT NULL, "price_at_addition" numeric, "price_tiers" "jsonb", "cart_items_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL, "added_at" timestamp with time zone DEFAULT "now"(), "updated_at" timestamp with time zone DEFAULT "now"(), CONSTRAINT "cart_items_quantity_check" CHECK (("quantity" > 0)));
ALTER TABLE "public"."cart_items" OWNER TO "postgres";

CREATE TABLE IF NOT EXISTS "public"."carts" ("user_id" "uuid" NOT NULL, "cart_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL, "status" "text" DEFAULT 'active'::"text" NOT NULL, "created_at" timestamp with time zone DEFAULT "now"(), "updated_at" timestamp with time zone DEFAULT "now"());
ALTER TABLE "public"."carts" OWNER TO "postgres";

CREATE TABLE IF NOT EXISTS "public"."control_panel_users" ("usuario" "text" NOT NULL, "password_hash" "text" NOT NULL, "last_login" timestamp with time zone, "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL, "is_active" boolean DEFAULT true, "created_at" timestamp with time zone DEFAULT "now"(), "email" "text", "full_name" "text", "role" "text" DEFAULT 'admin'::"text", "twofa_secret" "text", "notes" "text", "created_by" "uuid", "updated_at" timestamp with time zone DEFAULT "now"(), "twofa_required" boolean DEFAULT true, "twofa_configured" boolean DEFAULT false, CONSTRAINT "control_panel_users_role_check" CHECK (("role" = 'admin'::"text")));
...
CREATE SEQUENCE IF NOT EXISTS "public"."ejemplo_id_seq" START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
ALTER SEQUENCE "public"."ejemplo_id_seq" OWNER TO "postgres";
ALTER SEQUENCE "public"."ejemplo_id_seq" OWNED BY "public"."ejemplo"."id";

CREATE TABLE IF NOT EXISTS "public"."khipu_webhook_logs" ("id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL, "payment_id" character varying, "transaction_id" character varying, "status" character varying, "webhook_data" "jsonb", "signature_header" "text", "processed" boolean DEFAULT false, "processed_at" timestamp with time zone, "error_message" "text", "created_at" timestamp with time zone DEFAULT "now"() NOT NULL);
ALTER TABLE "public"."khipu_webhook_logs" OWNER TO "postgres";

CREATE TABLE IF NOT EXISTS "public"."orders" ("id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL, "user_id" "uuid" NOT NULL, "items" "jsonb" NOT NULL, "subtotal" numeric DEFAULT 0 NOT NULL, "tax" numeric DEFAULT 0 NOT NULL, "shipping" numeric DEFAULT 0 NOT NULL, "total" numeric DEFAULT 0 NOT NULL, "currency" character varying DEFAULT 'CLP'::character varying NOT NULL, "status" character varying DEFAULT 'pending'::character varying NOT NULL, "payment_method" character varying, "payment_status" character varying DEFAULT 'pending'::character varying NOT NULL, "shipping_address" "jsonb", "billing_address" "jsonb", "created_at" timestamp with time zone DEFAULT "now"() NOT NULL, "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL, "khipu_payment_id" character varying, "khipu_transaction_id" character varying, "khipu_payment_url" "text", "khipu_expires_at" timestamp with time zone, "paid_at" timestamp with time zone);
ALTER TABLE "public"."orders" OWNER TO "postgres";

CREATE TABLE IF NOT EXISTS "public"."payment_transactions" ("id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL, "order_id" "uuid" NOT NULL, "payment_method" character varying NOT NULL, "external_payment_id" character varying, "external_transaction_id" character varying, "amount" numeric NOT NULL, "currency" character varying DEFAULT 'CLP'::character varying NOT NULL, "status" character varying DEFAULT 'pending'::character varying NOT NULL, "gateway_response" "jsonb", "created_at" timestamp with time zone DEFAULT "now"() NOT NULL, "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL, "processed_at" timestamp with time zone);
...
CREATE TABLE IF NOT EXISTS "public"."products" ("productnm" "text" NOT NULL, "supplier_id" "uuid", "category" character varying, "description" "text", "max_quantity" integer, "productid" "uuid" DEFAULT "gen_random_uuid"() NOT NULL, "productqty" integer DEFAULT 0 NOT NULL, "price" numeric DEFAULT 0.00 NOT NULL, "minimum_purchase" integer DEFAULT 1, "negotiable" boolean DEFAULT false, "product_type" character varying DEFAULT 'general'::character varying, "min_quantity" integer DEFAULT 1, "spec_name" character varying DEFAULT 'N/A'::character varying NOT NULL, "spec_value" "text" DEFAULT 'N/A'::"text" NOT NULL, "is_active" boolean DEFAULT true, "createddt" timestamp with time zone DEFAULT "now"() NOT NULL, "updateddt" timestamp with time zone DEFAULT "now"() NOT NULL);
...
CREATE TABLE IF NOT EXISTS "public"."sales" ("user_id" "uuid" NOT NULL, "amount" numeric NOT NULL, "trx_date" timestamp with time zone DEFAULT "now"(), "trx_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL);
...
CREATE TABLE IF NOT EXISTS "public"."users" ("rut" character varying, "user_id" "uuid" NOT NULL, "email" "text" NOT NULL, "user_nm" character varying NOT NULL, "phone_nbr" character varying, "country" "text" NOT NULL, "logo_url" "text", "main_supplier" boolean DEFAULT false NOT NULL, "createdt" timestamp with time zone DEFAULT "now"() NOT NULL, "updatedt" timestamp with time zone DEFAULT "now"() NOT NULL, "descripcion_proveedor" "text", "banned" boolean DEFAULT false NOT NULL, "banned_at" timestamp with time zone, "banned_reason" "text", "verified" boolean DEFAULT false NOT NULL, "verified_at" timestamp with time zone, "verified_by" "uuid", "last_ip" "text");
...
ALTER TABLE ONLY "public"."admin_sessions" ADD CONSTRAINT "admin_sessions_pkey" PRIMARY KEY ("session_id");
ALTER TABLE ONLY "public"."bank_info" ADD CONSTRAINT "bank_info_user_id_key" UNIQUE ("user_id");
ALTER TABLE ONLY "public"."banned_ips" ADD CONSTRAINT "banned_ips_pkey" PRIMARY KEY ("ip");
ALTER TABLE ONLY "public"."billing_info" ADD CONSTRAINT "billing_info_user_id_key" UNIQUE ("user_id");
ALTER TABLE ONLY "public"."cart_items" ADD CONSTRAINT "cart_items_pkey" PRIMARY KEY ("cart_items_id");
ALTER TABLE ONLY "public"."carts" ADD CONSTRAINT "carts_pkey" PRIMARY KEY ("cart_id");
ALTER TABLE ONLY "public"."control_panel_users" ADD CONSTRAINT "control_panel_users_email_key" UNIQUE ("email");
ALTER TABLE ONLY "public"."control_panel_users" ADD CONSTRAINT "control_panel_users_pkey" PRIMARY KEY ("id");
ALTER TABLE ONLY "public"."control_panel_users" ADD CONSTRAINT "control_panel_users_usuario_key" UNIQUE ("usuario");
ALTER TABLE ONLY "public"."ejemplo" ADD CONSTRAINT "ejemplo_pkey" PRIMARY KEY ("id");
ALTER TABLE ONLY "public"."khipu_webhook_logs" ADD CONSTRAINT "khipu_webhook_logs_pkey" PRIMARY KEY ("id");
ALTER TABLE ONLY "public"."orders" ADD CONSTRAINT "orders_pkey" PRIMARY KEY ("id");
ALTER TABLE ONLY "public"."payment_transactions" ADD CONSTRAINT "payment_transactions_pkey" PRIMARY KEY ("id");
ALTER TABLE ONLY "public"."product_delivery_regions" ADD CONSTRAINT "product_delivery_regions_pkey" PRIMARY KEY ("id");
ALTER TABLE ONLY "public"."product_delivery_regions" ADD CONSTRAINT "product_delivery_regions_product_id_region_key" UNIQUE ("product_id", "region");
ALTER TABLE ONLY "public"."product_quantity_ranges" ADD CONSTRAINT "product_quantity_ranges_pkey" PRIMARY KEY ("product_qty_id");
ALTER TABLE ONLY "public"."products" ADD CONSTRAINT "products_pkey" PRIMARY KEY ("productid");
ALTER TABLE ONLY "public"."request_products" ADD CONSTRAINT "request_products_pkey" PRIMARY KEY ("request_product_id");
ALTER TABLE ONLY "public"."requests" ADD CONSTRAINT "requests_pkey" PRIMARY KEY ("request_id");
ALTER TABLE ONLY "public"."sales" ADD CONSTRAINT "sales_pkey" PRIMARY KEY ("trx_id");
ALTER TABLE ONLY "public"."shipping_info" ADD CONSTRAINT "shipping_info_user_id_key" UNIQUE ("user_id");
