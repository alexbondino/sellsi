...
-- 1. Eliminar triggers dependientes
...
BEGIN
  ...
  INSERT INTO public.users (
    user_id, email, user_nm, main_supplier, phone_nbr, country, createdt, updatedt
  ) VALUES (
    NEW.id,
    NEW.email,
...
CREATE FUNCTION public.get_banned_users() RETURNS TABLE(
  user_id uuid, user_name text, email text,
...
CREATE FUNCTION public.get_user_ban_info(target_user_id uuid) RETURNS TABLE(
  user_id uuid, user_name text, email text,
  is_banned boolean, banned_at timestamptz,
  banned_reason text, days_banned integer
) LANGUAGE plpgsql AS $$
BEGIN
  RETURN QUERY
    SELECT u.id,
           ...
           CASE
             ...
             ELSE 0 END::integer AS days_banned
    FROM auth.users u
...
-- 5. Tablas, secuencias, restricciones, relaciones, permisos:
...
GRANT ALL ON FUNCTION public.get_banned_users() TO anon, authenticated, service_role;
GRANT ALL ON FUNCTION public.get_user_ban_info(uuid) TO anon, authenticated, service_role;
GRANT ALL ON FUNCTION public.handle_new_user() TO anon, authenticated, service_role;
GRANT ALL ON FUNCTION public.handle_user_update() TO anon, authenticated, service_role;
GRANT ALL ON FUNCTION public.unban_user(uuid,text) TO anon, authenticated, service_role;
...
GRANT ALL ON TABLE public.bank_info TO anon, authenticated, service_role;
GRANT ALL ON TABLE public.banned_ips TO anon, authenticated, service_role;
GRANT ALL ON TABLE public.billing_info TO anon, authenticated, service_role;
...
GRANT ALL ON TABLE public.control_panel_users TO anon, authenticated, service_role;
GRANT ALL ON TABLE public.ejemplo TO anon, authenticated, service_role;
GRANT ALL ON SEQUENCE public.ejemplo_id_seq TO anon, authenticated, service_role;
GRANT ALL ON TABLE public.khipu_webhook_logs TO anon, authenticated, service_role;
GRANT ALL ON TABLE public.orders TO anon, authenticated, service_role;
GRANT ALL ON TABLE public.payment_transactions TO anon, authenticated, service_role;
GRANT ALL ON TABLE public.product_delivery_regions TO anon, authenticated, service_role;
GRANT ALL ON TABLE public.product_images TO anon, authenticated, service_role;
GRANT ALL ON TABLE public.product_quantity_ranges TO anon, authenticated, service_role;
GRANT ALL ON TABLE public.products TO anon, authenticated, service_role;
GRANT ALL ON TABLE public.request_products TO anon, authenticated, service_role;
...
GRANT ALL ON TABLE public.shipping_info TO anon, authenticated, service_role;
